...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  size_t out_index = 0;
  size_t current_index = gid;
  for (size_t i = 0; i < shape.size; i ++ ) {
    if(i == shape.size - 1) {
        out_index += current_index * strides.data[i];
        break;
    }
    size_t div = 1;
    ...
    size_t count = current_index / div;
    out_index += count * strides.data[i];
    current_index = current_index % div;
  }

   out[gid] = a[out_index + offset];

  // need to compute index based on gid
  // and then copy data from index to out
  // so how to define relationships between these 2 index
  // gid is index of out
  // so we need to compute
  // so, gid need to div by shape I think. well....
  // (2, 3, 5) -> 0: 0<5,so 0 is (0, 0, 0),
  // (15, 5, 0) -> 6: (0, 1, 1) 20(1, 1, 0)
